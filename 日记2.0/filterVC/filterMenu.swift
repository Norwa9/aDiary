...
class filterMenu: UIView {
    ...
    func configureUI(){
        ...
        //buttons
        for i in 0..<3 {
            let button = moodButton(frame: CGRect(x: 130 + CGFloat(i) * 50, y: 61, width: 40, height: 40))
    ...
    @IBAction func done(){
        ...
        let dataSource = diariesForConditions(keywords: keywords,selectedMood: selectedMood, selectedTags: selectedTags, numsToShow: selectedNum)
        monthVC.configureDataSource(dataSource:dataSource ,configFilteredDiariesOnly: true)
    }
